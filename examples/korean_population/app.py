"""
Korean Population & Apartment Market · Cortex Analyst Chat
-----------------------------------------------------------
A minimal Streamlit chat app that talks to the Snowflake Cortex Analyst REST API
using the Richgo apartment price and population migration semantic model.
...
def _render_results(df) -> None:
    ...
    date_cols = [c for c in df.columns if re.search(r"date|yyyymm|ymd|time", c, re.I)]
...
st.set_page_config(
    page_title="Korean Population & Apartments · Cortex Analyst",
    page_icon="🏘️",
    layout="wide",
)
st.markdown("""
